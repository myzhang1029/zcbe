...
int try_build_proj(char *proj_name)
{
    ...
    {
        fprintf(stderr,
                "try_build_proj: circular dependency detected while processing "
                "%s\n",
                proj_name);
        return 1;
    }
    ...
    {
        fprintf(stderr, "try_build_proj: invalid mapping.toml found: no "
                        "mapping table\n");
        toml_free(mapping);
        return 4;
    }
    ...
    {
        fprintf(stderr,
                "try_build_proj: project \"%s\" not found in mapping.toml\n",
                proj_name);
        toml_free(mapping);
        return 4;
    }
